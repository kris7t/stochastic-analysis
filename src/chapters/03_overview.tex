...
stochastic Petri net (\textls{SPN}) formalism \textbf{TODO
  Cite}, while engineering measures to be calculated are expressed as
\textls{SPN} performance measures. Both explicit and symbolic state
space exploration and storage is supported, including symbolic hierarchical
state space decomposition for block Kronecker generator
matrices.
...
\item The \emph{stochastic Petri net analysis} modules translate the
  ...
  defined by Computation Tree Logic (\textls{CTL}) are also evaluated
  by the symbolic analysis component. Therefore, \textls{CTL} rewards
...
\begin{table}
  ...
  \begin{tabular}{@{}llcccc@{}}
    ...
    \textls{IDR($s$)STAB($l$)} &
      ...
      & \multicolumn{4}{c@{}}{Ongoing research} \\
    \bottomrule
  \end{tabular}
...
of a solver based on \textls{IDR($s$)STAB($l$)} \textbf{TODO Cite}, a
Krylov subspace algorithm which generalizes \textls{BiCGSTAB}. As the
