...
One of the bechmark models was the modified version of stochastic \emph{SharedResource} system (presented in \cref{fig:background:spn:sharedresource}). We added three more nodes to the system and modified some of its parameters along two dimensions. On one hand we increased the number of reachable states by adding more resources and local processes to the model. On the other hand we changed the rates of transitions in the model resulting in changes in it's stochastic  behaviour.

TODO: állapottér méretek

\subsubsection{Kanban}

We used the \SPN\ version of the \emph{kanban} system \citep{ciardo2003logical} as the other benchmark model. The model was scaled by modifying the available resources at each stage of the model resulting in an increase in the size of the state space.

TODO: állapottér méretek

\subsubsection{Results}

\textbf{TODO: nagyobb modellek teljesítménye sparse Vs. BK}
...
In this section we present some of the state of the art modeling and analysis tools and their features without being exhaustive.
