...
\begin{algorithm}
  ...
  $U \gets A_U + I$\;
  \KwRet{$L, U$}\;
  \caption{Crout's \textls{LU} decomposition.}
...
Minimum Residual~(\textls{GMRES}), the the Conjugate Gradient and
Conjugate Residual methods, the BiConjugate Gradient algorithm and
Conjugate Gradient Squared~(\textls{CGS}). \textbf{TODO Cite}
...
\Cref{alg:algorithms:iterative:bicgstab} shows the pseudocode for
...
\cref{eq:algorithms:transient:uniformization} to compute $\vec{L}(t)$
...
\Cref{eq:algorithms:transient:uniformization-impl} is implemented by
\cref{alg:algorithms:transient:uniformization}. The algorithm performs
...
\begin{algorithm}
  ...
  \KwAllocate{$\vec{w} \in \RR^{k_{\text{right}} -
      k_{\text{left}}}$}\;
  \lFor{$j \gets k_{\text{left}}$ \KwTo $k_{\text{right}}$}{%
...
instead (\cref{alg:algorithms:transient:poisson}), which avoids
...
\cref{alg:algorithms:transient:uniformization}, thus a large right
...
\end{equation}
which advance the time together by $h_k$. The constant
...
tolerance $\tau$, since \cref{eq:algorithms:transient:trbdf-lte} is
...
\cref{alg:algorithms:transient:trbdf2}.
...
from \cref{sec:algorithms:solvers} may be used except power iteration,
...
$\neg \ctlAX \KwTrue$ is added to make \ref{eq:algorithms:mtff:gamma}
irreducible.
...
\eqref{eq:algorithms:mtff:gamma}.
