...
linear equation $\vec{x} Q = \vec{b}$ that satisfy the
Petrov--Galerkin conditions \textbf{TODO Cite}
\begin{equation}
  \label{eq:algorithms:krylov:galerkin}
  \vec{x}_k \in \mathcal{K}_k, \quad \vec{r}_k = \vec{b} - \vec{x}_k Q
...
\begin{equation}
  \mathcal{K}_k = \mathcal{K}_{k}(Q, \vec{r}_0) = \Span \{ \vec{r}_0,
  \vec{r}_0 Q, \vec{r}_0 Q^2, \ldots, \vec{r}_0 Q^{k - 1} \} \text,
\end{equation}
where $\mathcal{K}_{k}(Q, \vec{r}_0)$ is the \emph{$k$th Krylov
  subspace} of $Q$ and the initial residual $\vec{r}_0 = \vec{b} -
\vec{x}_0 Q$.

The smallest $m \in \NN$ such that
$\dim \mathcal{K}_m(Q, \vec{r}_0) = \dim \mathcal{K}_{m + 1}(Q,
\vec{r}_0)$
is called the \emph{grade} of $Q$ with respect to $\vec{r}_0$. Hence
$k \le m$ implies $\dim \mathcal{K}_k(Q, \vec{r}_0) = k$. Krylov
...
If $Q \in \RR^{n \times n}$ is of full rank and grade, Krylov subspace
...
Note that problems may arise when $Q$ is singular, which may worsen
...
\begin{equation}
  ...
  \mathcal{L}_k = \mathcal{K}_k Q = \{ \vec{r}_0Q, \vec{r}_0Q^2,
  \ldots, \vec{r}_0 Q^k \} \text.
\end{equation}
...
\begin{equation}
  ...
  \vec{x}_k \in \mathcal{K}_k \text{ such that } \vec{r}_k = \vec{b} - \vec{x}_k Q \perp
...
\begin{equation}
  ...
  \mathcal{L}_k = \mathcal{K}_k(Q^\T, \tilde{\vec{r}}_0) \cdot
  (\Omega_k(Q)^\T)^{-1}, \quad
  \Omega_k(Q) = \begin{cases}
    \Omega_{k - 1}(Q) \cdot (I - \omega_k Q) & \text{if $k \ge 1$,} \\
    I & \text{if $k = 0$.}
  \end{cases}
\end{equation}
...
\begin{equation}
  \mathcal{G}_k = \mathcal{S}(\Omega_k, Q, \tilde{\vec{r}}_0) = \{
  \vec{v} \cdot \Omega_k(Q) : \vec{v} \perp \mathcal{K}_k(Q^\T,
  \tilde{\vec{r}}_0) \} \text,
\end{equation}
where $\mathcal{S}(\Omega, Q, \tilde{\vec{r}}_0)$ is the
$\Omega$\kern0.5pt th \emph{Sonneveld subspace} generated by $Q$ and
...
\ref{nl:algorithms:bicgtab:bicg-start}--\ref{nl:algorithms:bicgtab:bicg-end}
calculates a residual $\vec{t} \in \mathcal{G}_{k - 1} Q$ and its
associated approximate solution $\vec{x} \in \mathcal{K}_k$. The
\textls{GMRES}($1$) part in lines~%
\ref{nl:algorithms:bicgtab:gm-start}--\ref{nl:algorithms:bicgtab:gm-end}
...
\item The first idea comes from \textls{IDR}($s$)%
  ...
  $\vec{r}_k$
  \begin{equation}
    \vec{r}_k \in \mathcal{G}_k = \mathcal{S}(\Omega_k, Q, \widetilde{R}_0) =
    \{ \vec{v} \cdot \Omega_k(Q) : \vec{v} \perp \mathcal{K}_k(Q,
    \tilde{R}_0) \}\text,
  \end{equation}
  where $\mathcal{K}_k(Q, \tilde{R}_0)$ is the $k$th \emph{row} Krylov
  subspace of $Q \in \RR^{n \times n}$ with respect to
  $\tilde{R}_0 \in \RR^{s \times n}$
  \begin{equation}
    \mathcal{K}_k(Q, \widetilde{R}_0) = \Span \{ \tilde{\vec{r}}_0[i],
    \tilde{\vec{r}}_0[i] Q, \ldots, \tilde{\vec{r}}_0[i] Q^{k - 1} : i =
    0, 1, \ldots, s - 1 \}
  \end{equation}
...
\item The second generalization, which is called
  ...
  \begin{equation}
    \Omega_k(Q) = \Omega_{k - 1}(Q) \cdot (I - \gamma[0] Q - \gamma[1] Q^2
    - \cdots - \gamma[\ell - 1] Q^{\ell}) \text,
  \end{equation}
...
\end{equation}}
for $j = 1, 2, \ldots, \ell$. After the repetition step is complete,
$\vec{U}$ and $\vec{V}$ are swapped and the process starts again with
increased $j$ or the \textls{GMRES}($\ell$) part commences. Symbols
with a subscript ``$-$'' sign refer to vectors from the previous
repetition step.
...
$\mathcal{K}_s(Q \Pi_j, \vec{r} Q^j \Pi_j)$ after the $j$th repetition
step.

The relationships $\vec{r}[i + 1] = \vec{r}{i} Q$,
$\vec{u}[i + 1, q] = \vec{u}[i, q] Q$,
$\vec{v}[i + 1, q] = \vec{v}[i, q]$ are maintained throughout the
algorithm via the projections. This is signified by the gray
{\color{black!50}$\downarrow Q$} arrows in
\cref{eq:algorithms:idrstab:projections}. Notice that this means
$\vec{r}[0] Q^i = \vec{r}[i]$ and $U[0, \cdot] Q^i = U[i, \cdot]$.

In the case of $\vec{r}[j]$ and $\vec{v}[j + 1, q]$, a matrix
multiplication is performed as shown by the $\downarrow Q$
