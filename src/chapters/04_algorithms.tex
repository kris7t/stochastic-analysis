...
In Jacobi over-relaxation, parallelization of both the matrix
...
the convergence behavior. This is the case in \textls{CTMC} analysis,
...
The convergence behavior of full \textls{GMRES} is often
excellent. However, due to impractical memory requirements, we did not
implement \textls{GMRES} as a numerical solver in our framework.  We
...
behavior and low memory requirements. \textls{BiCGSTAB} only requires
...
procedures. The bi-conjugate gradient (\textls{Bi-CG}) part in lines~%
...
\begin{algorithm}
  \For(\tcp*[f]{For every repetition step}){$j \gets 1$ \KwTo $\ell$}{
    %\AlgSection{Calculate the new residuals}
...
are highlighted with \HlNode{shaded} line numbers.
...
requirements of \textls{IDR}($s$)\textls{STAB}($\ell$) are quite high,
...
\textls{IDR} formulation it is less numerically stable.
...
\begin{algorithm}
  ...
  \KwOut{instantaneous or accumulated probability vector $\vec{x} \in
    \RR^n$}
