...
work. At first, Petri net based formalisms are introduced, which is
the modelling language our framework uses to express stochastic
models. Then the basic stochastic modelling background is discussed
and Kronecker algebra is also introduced to give a base for the later sections.

\section{Petri nets}
\label{chap:background:sec:petri-nets}
...
If $M\,\tranto{t}$ holds the transition $t$ can be \emph{fired} to get a
...
The reachable state space $\RS$ is finite if and only if the Petri net
is bounded.
...
\begin{example}
  ...
  tokens on $p_{\text{H$_{2}$O}}$ are the products of the reaction. Now $t$ is no
  longer enabled.
\end{example}
...
\begin{runningExample}
  ...
  consumers. Each consumer can be in one of the following states:
  $C_i$ \paren{calculating locally}, $W_i$ \paren{waiting for
    resource} and $S_i$ \paren{using shared resource}. The
  ...
  %In contrast, we cannot compute dependability
...
Inhibitor arcs are widely used extensions of Petri nets that can disable transitions even when the firing rule defined in \cref{chap:background:sec:petri-nets} is satisfied. 
...
%enablement. 
This modification gives Petri nets expressive power
...
\begin{equation}
  ...
  M\,\tranto{t} \Longleftrightarrow \text{\strut$M(p) \ge W(p, t)$ for all
    ...
    \inharc{t}$.}
\end{equation}
The firing rule \eqref{eq:background:petri:fire} remains unchanged in
...
the study of dependability or reliability, the explicit
...
Stochastic Petri Nets extend Petri nets by assigning
...
\item The reachable state space of the Petri net is explored. We
  associate consecutive natural numbers with the states such that
  the state space is
  \begin{equation}
  ...
  - 1\}$ to refer to states (in \textls{CTMC} terminology) and
  markings (in \textls{SPN} terminology) of the model interchangably.
...
\begin{runningExample}
  ...
  $0.5$, $1.0$ and $1.1$, respectively. The vector $\vec{\uptheta} = (0.625, 1.25) \in \RR^2$ of model parameters is introduced such
  ...
  state graph along with the transition rates of the \CTMC\ is
  shown in \cref{fig:background:spn:sharedresouce-ctmc}. The generator matrix is (also depicting state indices):
...
The stochastic reward net formalism is an extension of stochastic Petri
nets that allows the definition of performance measures on the net
level for use in the stochastic analysis workflow.
...
distribution of the accumulated reward $Y(t)$ cannot be derived by this method~\citep{RACZ99a}.
...
In this section we define the base formalism of the decomposition algorithm introduced in \cref{chap:genstor}.
...
\begin{dfn}
  ...
  is the partitioning of the set of places and $P = \loc{P}{0} \cup \loc{P}{1} \cup \cdots \cup \loc{P}{J -
...
\begin{itemize}
\item $\loc{P}{j}$ is the corresponding set from the partitioning of the original net.
\item $\loc{T}{j}$ contains the local transition $\loc{T_L}{j}$ and
  synchronization transitions $\loc{T_S}{j}$.

  A transition is \emph{local} to $\LN{j}$ if it only affects places
  in $\loc{P}{j}$, that is,
  \begin{equation}
...
considered when a local net $\LNI{j}$ is constructed. The set
...
that the enabling of local transitions only depends on the marking
...
The set of all synchronization transitions is denoted as
$T_S = \bigcup_{j = 0}^{J - 1} \loc{T_S}{j}$. The \emph{support} of
the transition $t \in T$ is the set of components it
is adjacent to, $\supp t = \{j : t \in \loc{T}{j}\}$.
...
\begin{runningExample}
  ...
  respectively, while $a_1$, $d_1$, $a_2$ and $d_2$ synchronize $\LN{2}$ and the local net associated with their consumers.
\end{runningExample}
...
Cartesian product of the local reachable state spaces of its components
\begin{equation}
...
notation
\begin{equation}
  \label{eq:background:sspn:concat}
...
Kronecker algebra defines the building blocks of the decomposition algorithm being introduced in \cref{chap:genstor}.
